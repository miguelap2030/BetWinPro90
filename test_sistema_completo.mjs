 ...
 * 
 * Uso: node test_sistema_completo.mjs
 */
...
// ============================================================================
async function signUp(email, password, referralCode = null, sponsorId = null) {
  ...
  try {
    ...
    // Paso 4: Crear perfil usando SQL directo (para evitar RLS)
    log.info(`Creando perfil con sponsor_id: ${sponsorId}`);
    ...
    // Si hay sponsor, crear el referido directamente
    if (sponsorId) {
      ...
      if (refError) {
        log.warn(`Advertencia al crear referido: ${refError.message}`);
      } else {
        log.success(`✅ Referido registrado correctamente`);
        
        // Actualizar contador de referidos del sponsor
        await supabase.rpc('update_referral_count', { p_sponsor_id: sponsorId });
      }
    }
...
async function runTests() {
  ...
  const TIMESTAMP = Date.now(); // Para emails únicos
  ...
  try {
    ...
    const user1 = await signUp(`admin.test+${TIMESTAMP}@gmail.com`, PASSWORD);
    console.log('');
    
    // Usuario 2: referido de admin (pasamos sponsorId directamente)
    log.info('Creando Usuario 2 (referido de admin)...');
    const user2 = await signUp(
      `usuario2.test+${TIMESTAMP}@gmail.com`,
      PASSWORD,
      user1.referralCode,
      user1.userId  // sponsorId directo
    );
    console.log('');
    ...
    const user3 = await signUp(
      `usuario3.test+${TIMESTAMP}@gmail.com`,
      PASSWORD,
      user2.referralCode,
      user2.userId  // sponsorId directo
    );
    console.log('');
