-- ============================================================================
-- CORRECCIÓN DE TABLA ADMIN_DASHBOARD
-- ============================================================================
-- Script corregido basado en la estructura real de la BD
-- ============================================================================
...
-- ============================================================================
-- 4. FUNCIÓN PARA ACTUALIZAR ESTADÍSTICAS (CORREGIDA)
-- ============================================================================
...
BEGIN
  UPDATE admin_dashboard SET
    -- Estadísticas de Usuarios (usando is_active en lugar de last_sign_in_at)
    ...
    total_invertido = (SELECT COALESCE(SUM(balance_invested), 0) FROM wallets),
    total_disponible = (SELECT COALESCE(SUM(balance_available), 0) FROM wallets),
    total_retirado = (SELECT COALESCE(SUM(balance_withdrawn), 0) FROM wallets),
    total_comisiones = (SELECT COALESCE(SUM(amount), 0) FROM mlm_commissions),
    ...
    comisiones_pendientes = (SELECT COALESCE(SUM(amount), 0) FROM mlm_commissions WHERE status = 'pending'),
...
-- Trigger para profiles (usuarios)
...
-- Política para SELECT: Solo admins pueden ver (usando profiles.id)
...
-- ============================================================================
-- 7. FUNCIÓN RPC PARA OBTENER ESTADÍSTICAS (CORREGIDA)
-- ============================================================================
...
-- ============================================================================
-- 8. FUNCIÓN PARA ACTUALIZACIÓN MANUAL
-- ============================================================================
...
BEGIN
  ...
  PERFORM update_admin_dashboard();
END;
$$ LANGUAGE plpgsql;
